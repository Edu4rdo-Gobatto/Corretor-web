...
export default function LeadFormModal({ property, onClose }: { property: Property; onClose: () => void }) {
  ...
  function submit(event: FormEvent<HTMLFormElement>) {
    ...
    if (!contactOpened && !isDemo) { window.open(contactUrl, '_blank', 'noopener,noreferrer'); setContactOpened(true); }
  ...
  return <Dialog title={saved ? 'Obrigado pelo seu interesse.' : 'Vamos falar sobre este espaço?'} onClose={onClose}>
    {saved ? <div className={styles.success}><CheckCircle2 size={32}/><p>{isDemo ? 'Contato salvo na demonstração. Você pode consultá-lo em Leads, no painel.' : 'Seu interesse foi registrado. Continue a conversa com o corretor pelo WhatsApp.'}</p>{!isDemo && <a className="button" href={contactUrl} target="_blank" rel="noopener noreferrer">Continuar no WhatsApp <ArrowUpRight size={17}/></a>}<button className="buttonSecondary" onClick={onClose}>Concluir</button></div> : <><p className="muted">{property.title}<br/>Atendimento com {property.agent.name}.</p>{isDemo && <p className={styles.demoNote}>Demonstração: nenhum WhatsApp será aberto e nenhum contato real será enviado.</p>}<form onSubmit={submit} className={styles.form} noValidate>
      ...
      <button className="button" type="submit" disabled={saving}>{saving ? 'Registrando contato…' : failure ? 'Tentar registrar novamente' : isDemo ? 'Simular contato' : 'Falar pelo WhatsApp'}<ArrowUpRight size={17}/></button>
