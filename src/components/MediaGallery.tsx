...
export default function MediaGallery({ media, title }: { media: PropertyMedia[]; title: string }) {
  ...
  return <div className={styles.gallery}><div className={styles.main} tabIndex={ordered.length > 1 ? 0 : undefined} aria-label={ordered.length > 1 ? 'Galeria de imagens. Use as setas para navegar.' : undefined} onKeyDown={event => { if (ordered.length < 2) return; if (event.key === 'ArrowRight') setIndex(value => (value + 1) % ordered.length); if (event.key === 'ArrowLeft') setIndex(value => (value - 1 + ordered.length) % ordered.length); }}><MediaView media={current} title={title}/>{current.type === 'IMAGE' && <button className={styles.expand} onClick={() => setExpanded(true)}><Expand size={16}/> Ampliar foto</button>}<span className={styles.counter}>{index + 1} / {ordered.length}</span></div>{ordered.length > 1 && <div className={styles.thumbnails}>{ordered.map((item, position) => <button key={item.id} className={index === position ? styles.active : ''} onClick={() => setIndex(position)} aria-label={`Ver ${item.type === 'IMAGE' ? 'foto' : 'vídeo'} ${position + 1}`} aria-pressed={index === position}>{item.type === 'IMAGE' ? <img src={item.url} alt="" loading="lazy"/> : <Play size={24}/>}</button>)}</div>}{expanded && <Dialog title={title} onClose={() => setExpanded(false)}><img className={styles.full} src={current.url} alt={title}/></Dialog>}</div>;
}
