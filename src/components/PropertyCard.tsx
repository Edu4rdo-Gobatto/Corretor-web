...
export default function PropertyCard({ property }: { property: Property }) {
  ...
  return <article className="min-w-0">
    ...
    <div className="pt-5"><p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-brand">{property.typeName || propertyType[property.type] || 'Imóvel'}</p><h3 className="mb-2 text-[21px] max-[560px]:text-[19px]"><Link to={propertyUrl(property.slug)} className="no-underline hover:underline">{property.title}</Link></h3><p className="mb-[17px] flex items-center gap-[5px] text-sm text-muted"><MapPin size={14}/>{property.neighborhood}, {property.addressCity}</p><div className="flex items-center justify-between gap-2.5 border-t border-line pt-[15px]"><span className="flex items-center gap-2 text-sm"><Maximize2 size={15}/>{area(property.usableArea)}</span><p className="m-0 text-right font-display whitespace-normal [overflow-wrap:anywhere]"><strong className="text-[20px] font-semibold text-brand">{money(property.price)}</strong>{property.purpose === 'LOCACAO' && <span className="font-sans text-xs text-muted"> /mês</span>}</p></div></div>
  </article>;
}
