...
export default function PropertyCard({ property }: { property: Property }) {
  ...
  return <article className="min-w-0">
    <Link to={propertyUrl(property.slug)} className="group relative block aspect-[1.47/1] overflow-hidden rounded-lg border-b-[3px] border-b-gold bg-[#E2E5DC] max-[560px]:aspect-[4/3]" aria-label={`Ver ${property.title}`}>
      ...
      <span className="absolute left-[17px] top-[17px] rounded-sm border-b-2 border-gold bg-navy px-[11px] py-[6px] text-[11px] font-semibold uppercase tracking-[0.065em] text-white">{property.purpose === 'LOCACAO' ? 'Para alugar' : 'À venda'}</span><span className="absolute bottom-[14px] right-[14px] z-[1] grid h-11 w-11 place-items-center rounded-full bg-paper transition group-hover:translate-x-[2px] group-hover:-translate-y-[2px] group-hover:bg-gold-soft"><ArrowUpRight size={19}/></span>
    </Link>
    <div className="pt-5"><p className="mb-2 text-[11px] font-semibold uppercase tracking-[0.12em] text-brand">{propertyType[property.type]}</p><h3 className="mb-2 text-[21px] max-[560px]:text-[19px]"><Link to={propertyUrl(property.slug)} className="no-underline hover:underline">{property.title}</Link></h3><p className="mb-[17px] flex items-center gap-[5px] text-sm text-muted"><MapPin size={14}/>{property.neighborhood}, {property.addressCity}</p><div className="flex items-center justify-between gap-2.5 border-t border-line pt-[15px]"><span className="flex items-center gap-2 text-sm"><Maximize2 size={15}/>{area(property.usableArea)}</span><p className="m-0 text-right font-display whitespace-normal [overflow-wrap:anywhere]"><strong className="text-[20px] font-semibold text-brand">{money(property.price)}</strong>{property.purpose === 'LOCACAO' && <span className="font-sans text-xs text-muted"> /mês</span>}</p></div></div>
  </article>;
}
