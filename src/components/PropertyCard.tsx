...
export default function PropertyCard({ property }: { property: Property }) {
  ...
  return <article className={styles.card}>
    <Link to={propertyUrl(property.slug)} className={styles.imageLink} aria-label={`Ver ${property.title}`}>
      {showCover ? <img src={cover.url} alt={property.title} loading="lazy" decoding="async" onError={() => setImageFailed(true)}/> : <div className={styles.noPhoto}>Foto em breve</div>}
