...
export default function PublicLayout() {
  ...
  return <>
    ...
    <header className="sticky top-0 z-40 border-b border-line bg-paper shadow-[inset_0_-2px_0_var(--color-gold)]"><div className="container flex min-h-[104px] items-center justify-between gap-8 max-[900px]:gap-4 max-[800px]:relative max-[800px]:min-h-[84px]">
      <Link to={routes.home} className="relative z-[6] flex min-w-0 items-center gap-3 max-[900px]:gap-2 text-brand no-underline" aria-label={`${brand.name} — início`}><img src={brand.logo.asset} alt={`${brand.logo.first} — ${brand.logo.second}`} className="h-[48px] w-auto max-w-[230px] object-contain object-left mix-blend-multiply dark:mix-blend-screen max-[900px]:h-10 max-[900px]:max-w-[175px]" /></Link>
    ...
    <footer className="mt-12 border-t-2 border-gold bg-navy py-8 text-white md:mt-20">
      <div className="container grid gap-8 md:grid-cols-2 lg:grid-cols-3">
        ...
        <div className="grid grid-cols-2 gap-x-6 gap-y-1 text-sm [&_a]:flex [&_a]:min-h-11 [&_a]:items-center [&_a]:text-white [&_a:hover]:underline">
          <Link to={routes.home}>Imóveis comerciais</Link>
          <Link to={catalogUrl({purpose:'LOCACAO'})}>Alugar</Link>
          <Link to={catalogUrl({purpose:'VENDA'})}>Comprar</Link>
          <Link to={routes.privacy}>Política de privacidade</Link>
          <Link to={routes.devs}>Desenvolvedores</Link>
          <Link to={routes.admin}>Área do corretor</Link>
        </div>
        {(brand.contact.whatsapp || brand.contact.email || brand.contact.hours || brand.contact.address) && <address className="grid content-start gap-2 text-sm not-italic text-white/80">
          {brand.contact.whatsapp && <a className="min-h-11 text-white" href={`https://wa.me/${whatsappPhone(brand.contact.whatsapp)}`} target="_blank" rel="noopener noreferrer">WhatsApp: {brand.contact.whatsapp}</a>}
