...
export default function PublicLayout() {
  ...
  return <>
    ...
    <header className="border-b border-line bg-paper shadow-[inset_0_-2px_0_var(--color-gold)]"><div className="container flex min-h-[104px] items-center justify-between gap-8 max-[800px]:relative max-[800px]:min-h-[84px]">
      ...
      <nav id="public-navigation" className={`flex items-center gap-[30px] text-[15px] max-[800px]:gap-[18px] max-[800px]:text-sm max-[650px]:absolute max-[650px]:inset-x-0 max-[650px]:top-[84px] max-[650px]:z-[5] max-[650px]:flex-col max-[650px]:items-stretch max-[650px]:gap-1 max-[650px]:border-b-[3px] max-[650px]:border-b-gold max-[650px]:bg-navy max-[650px]:p-6 max-[650px]:text-white max-[650px]:shadow-[0_12px_16px_#0001] ${menuOpen ? 'max-[650px]:flex' : 'max-[650px]:hidden'} max-[650px]:[&_a]:flex max-[650px]:[&_a]:min-h-12 max-[650px]:[&_a]:items-center max-[650px]:[&_a]:font-display max-[650px]:[&_a]:text-[19px] [&_a]:no-underline [&_a:hover]:underline`} aria-label="Principal"><NavLink ref={firstLinkRef} to={routes.home} end onClick={() => closeMenu(false)}>Encontrar um imóvel</NavLink><Link to={catalogUrl({purpose:'LOCACAO'})} onClick={() => closeMenu(false)}>Alugar</Link><Link to={catalogUrl({purpose:'VENDA'})} onClick={() => closeMenu(false)}>Comprar</Link><Link to={routes.admin} onClick={() => closeMenu(false)} className="flex items-center gap-[14px] border-l border-line py-3 pl-8 max-[800px]:pl-[18px] max-[650px]:min-h-12 max-[650px]:border-l-0 max-[650px]:border-t max-[650px]:border-t-[#ffffff2e] max-[650px]:pl-0 max-[650px]:font-sans max-[650px]:text-base">Área do corretor <ArrowUpRight size={16}/></Link><p aria-hidden="true" className="m-0 hidden pt-2 text-[11px] font-semibold uppercase tracking-[0.18em] text-gold max-[650px]:block">CRECI {brand.creci}</p></nav>
    </div></header>
