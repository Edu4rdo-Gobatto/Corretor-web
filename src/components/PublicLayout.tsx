...
export default function PublicLayout() {
  ...
  return <>
    ...
    <header className={styles.header}><div className={`container ${styles.headerInner}`}>
      <Link to="/" className={styles.brand} aria-label={`${brand.name} — início`}><Building2 strokeWidth={1.3} size={34}/><span>{brand.logo.first}<span>{brand.logo.second}</span></span></Link>
