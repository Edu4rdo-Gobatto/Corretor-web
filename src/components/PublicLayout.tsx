...
export default function PublicLayout() {
  ...
  return <>
    ...
    <header className={styles.header}><div className={`container ${styles.headerInner}`}>
      ...
      <button className={`buttonGhost ${styles.menuToggle}`} aria-label={menuOpen ? 'Fechar menu' : 'Abrir menu'} aria-expanded={menuOpen} onClick={() => setMenuOpen(!menuOpen)}>{menuOpen ? <X/> : <Menu/>}</button>
      <nav className={`${styles.navigation} ${menuOpen ? styles.open : ''}`} aria-label="Principal" onClick={() => setMenuOpen(false)}><NavLink to="/">Encontrar um imóvel</NavLink><Link to="/?purpose=LOCACAO">Alugar</Link><Link to="/?purpose=VENDA">Comprar</Link><Link to="/admin" className={styles.adminLink}>Área do corretor <ArrowUpRight size={16}/></Link></nav>
    </div></header>
