...
const chipIdle = 'border-line bg-transparent text-muted hover:border-navy dark:hover:border-gold';
...
export default function Catalog() {
  ...
  return <>
    ...
    <div className="container relative"><form ref={formRef} onSubmit={search} className="rounded-[5px] border border-line bg-paper px-7 pt-[26px] pb-[15px] shadow-[0_14px_34px_rgb(36_40_36/0.08)] max-[560px]:px-[18px] max-[560px]:pt-5 max-[560px]:pb-[10px]" aria-label="Buscar imóveis">
      ...
      <datalist id="catalog-cities">{Array.from(new Map((properties?.items ?? []).map(item => [item.addressCity.trim().toLocaleLowerCase('pt-BR'), item.addressCity.trim()])).values()).filter(Boolean).sort((a, b) => a.localeCompare(b, 'pt-BR')).map(city => <option value={city} key={city}/>)}</datalist>
    </form></div>
