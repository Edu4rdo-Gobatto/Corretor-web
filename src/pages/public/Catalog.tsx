...
const fieldControl = 'mt-2 min-h-[34px] border-0 p-[2px_26px_2px_0] text-base max-[560px]:rounded-none max-[560px]:border-b max-[560px]:border-line max-[560px]:pb-2';
...
export default function Catalog() {
  ...
  return <>
    ...
    <section className="container grid grid-cols-[minmax(0,0.92fr)_minmax(0,1.08fr)] gap-16 pt-14 pb-[54px] max-[1100px]:gap-7 max-[800px]:gap-[22px] max-[800px]:pt-[30px] max-[560px]:grid-cols-1 max-[560px]:gap-7 max-[560px]:pb-[25px]">
      <div className="pt-6 max-[800px]:pt-[6px] max-[560px]:pt-0"><p className="eyebrow flex items-center"><span className="mr-3 inline-block h-[2px] w-[26px] bg-gold align-middle"/> Imóveis comerciais</p><h1 className="mt-[25px] mb-6 max-w-[650px] text-[clamp(35px,4vw,60px)] leading-[1.14] tracking-[-0.055em] max-[1100px]:text-[44px] max-[800px]:text-[36px] max-[560px]:mt-5 max-[560px]:text-[43px] min-[1550px]:text-[64px]">Imóveis comerciais para alugar e comprar <em className="font-normal text-navy">em {brand.region.name}.</em></h1><p className="mb-6 text-[17px] leading-[1.75] text-muted max-[800px]:text-[15px] max-[560px]:text-base">Encontre salas comerciais, lojas, galpões, prédios e terrenos.<br className="max-[800px]:hidden"/> Consulte as opções para o seu negócio em {brand.region.name}.</p><a className="inline-flex items-center gap-7 border-b-2 border-gold pb-2 text-sm font-semibold no-underline" href="#catalogo">Explore os imóveis <ArrowDown size={17}/></a><div className="mt-[38px] text-xs text-muted max-[1100px]:mt-6 max-[560px]:hidden"><span className="mr-[13px]">01 —</span> Um lugar para o que vem a seguir.</div></div>
      <div className="relative isolate mr-6 h-[470px] before:absolute before:top-[18px] before:-right-[18px] before:-bottom-[18px] before:left-[18px] before:-z-[1] before:rounded-[3px] before:border before:border-gold before:content-[''] max-[1100px]:h-[430px] max-[800px]:mr-[14px] max-[800px]:h-[400px] max-[560px]:h-[280px] min-[1550px]:h-[510px]"><img src="/assets/commercial-space-1200.webp" srcSet="/assets/commercial-space-640.webp 640w, /assets/commercial-space-960.webp 960w, /assets/commercial-space-1200.webp 1200w" sizes="(max-width: 560px) calc(100vw - 36px), (max-width: 800px) 50vw, 600px" width="1200" height="900" alt="Ambiente comercial iluminado, com mesas, vegetação e janelas amplas" className="h-full w-full rounded-[3px] object-cover shadow-[0_22px_48px_rgb(23_77_59/0.16)]" {...{ fetchpriority: "high" }}/><div className="absolute inset-x-0 bottom-0 flex items-center justify-between bg-[linear-gradient(transparent,#132E24E6)] px-[25px] pt-12 pb-5 text-white [text-shadow:0_1px_2px_#000]"><span className="text-[10px] tracking-[0.18em]">ESPAÇOS QUE INSPIRAM</span><ArrowUpRight size={19}/></div><div className="absolute top-0 right-[-27px] text-[9px] tracking-[0.17em] text-muted [writing-mode:vertical-rl] max-[800px]:right-[-20px]">TRABALHAR. CRIAR. CRESCER.</div></div>
    </section>
    <div className="container relative"><form ref={formRef} onSubmit={search} className="rounded-[5px] border border-line bg-paper px-7 pt-[26px] pb-[15px] shadow-[0_14px_34px_rgb(36_40_36/0.08)] max-[560px]:px-[18px] max-[560px]:pt-5 max-[560px]:pb-[10px]" aria-label="Buscar imóveis">
      <div className="grid grid-cols-[1fr_1fr_1.1fr_auto] items-end gap-[26px] max-[1100px]:gap-[18px] max-[800px]:grid-cols-2 max-[800px]:gap-5 max-[560px]:gap-x-[14px] max-[560px]:gap-y-[18px]"><label className="block border-r border-line pr-5 text-xs font-semibold max-[1100px]:pr-[10px] max-[800px]:border-0 max-[800px]:p-0">O que você procura?<select name="purpose" defaultValue={query.purpose || ''} className={fieldControl}><option value="">Alugar ou comprar</option><option value="LOCACAO">Quero alugar</option><option value="VENDA">Quero comprar</option></select></label><label className="block border-r border-line pr-5 text-xs font-semibold max-[1100px]:pr-[10px] max-[800px]:border-0 max-[800px]:p-0">Tipo de imóvel<select name="type" defaultValue={query.type || ''} className={fieldControl}><option value="">Todos os tipos</option>{Object.entries(propertyTypes).map(([key, label]) => <option value={key} key={key}>{label}</option>)}</select></label><label className="block text-xs font-semibold max-[560px]:col-span-full">Onde?<input name="city" placeholder="Cidade" defaultValue={query.city || ''} maxLength={100} className={fieldControl}/></label><button className="button mb-[3px] max-[1100px]:px-[15px] max-[1100px]:py-3 max-[560px]:col-span-full" type="submit"><Search size={17}/> Encontrar imóvel</button></div>
      <div className="mt-[19px] flex items-center gap-5 border-t border-[#EEEFE8] pt-3"><button type="button" className="inline-flex min-h-8 items-center gap-2 border-0 bg-transparent px-0 py-[6px] text-xs text-muted" aria-expanded={priceOpen} onClick={() => setPriceOpen(!priceOpen)}><SlidersHorizontal size={15}/> Faixa de preço</button>{(query.type || query.purpose || query.city || query.minPrice !== undefined || query.maxPrice !== undefined || query.page > 1) && <button type="button" className="inline-flex min-h-8 items-center gap-2 border-0 bg-transparent px-0 py-[6px] text-xs text-muted underline" onClick={() => { setParams({}); setFilterError(''); }}>Limpar filtros</button>}<span className="ml-auto text-xs text-muted max-[560px]:hidden">O endereço certo faz a diferença.</span></div>
