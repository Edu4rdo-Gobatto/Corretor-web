...
function show(currentAgent:Agent|null){vi.mocked(useAuth).mockReturnValue({agent:currentAgent,loading:false,login:vi.fn(),logout:vi.fn()});render(<MemoryRouter initialEntries={['/admin']}><Routes><Route path="/admin" element={<AdminLayout/>}><Route index element={<p>Conteúdo protegido</p>}/></Route><Route path="/admin/login" element={<p>Acesso à conta</p>}/></Routes></MemoryRouter>);}
afterEach(cleanup);
