...
export const routes = { home: '/', privacy: '/privacidade', admin: '/admin', login: '/admin/entrar', contacts: '/admin/contatos' };
...
export function normalizedUrl(path: string): string {
  ...
  const known = pathname === routes.privacy || /^\/imoveis\/[^/]+$/.test(pathname) || /^\/admin(?:\/(?:entrar|contatos|corretores|imoveis(?:\/(?:novo|[^/]+\/editar))?|(?:proprietarios|inquilinos|contratos)(?:\/[^/]+)?))?$/.test(pathname);
