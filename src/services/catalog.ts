...
export function readCatalogQuery(params: URLSearchParams): CatalogQuery {
  ...
  if (type && /^[a-zA-Z0-9][a-zA-Z0-9-]{0,119}$/.test(type)) query.type = type as PropertyType;
  if (purpose && /^[a-zA-Z0-9][a-zA-Z0-9-]{0,119}$/.test(purpose)) query.purpose = purpose as PropertyPurpose;
